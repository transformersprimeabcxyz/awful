...
namespace Awful.Core.Services
{
    ...
    {
        ForumData Forum { get; set; }
        IList<AwfulTag> Icons { get; set; }
        string FormKey { get; }
        string FormCookie { get; }
        string Title { get; set; }
        string Text { get; set; }
        AwfulTag SelectedIcon { get; set; }
    }

    public class NewThreadRequest : KollaSoft.PropertyChangedBase, AwfulThreadRequest
    {
        private AwfulTag _selectedIcon;
        private string _title;
        private string _text;

        public ForumData Forum { get; set; }
        
        public IList<AwfulTag> Icons { get; set; }
        public string Text
        {
    ...
    public class AwfulThreadCreatorService
    {
        ...
        private void InitiatePreview(AwfulThreadRequest data, Action<ActionResult, AwfulThreadPage> result)
        {
            var dispatch = Deployment.Current.Dispatcher;
            Logger.AddEntry("ThreadReplyService - Initiating Reply...");
            ...
            {
                string url = NEW_THREAD_URL;
                HttpWebRequest request = AwfulWebRequest.CreateFormDataPostRequest(url, PREVIEW_THREAD_CONTENT_TYPE);
                ...
                request.BeginGetRequestStream((asyncResult) =>
                {
                    ...
                    {
                        Logger.AddEntry("ThreadCreatorService - Send failed @ GetRequest.");
                        Deployment.Current.Dispatcher.BeginInvoke(() => { result(ActionResult.Failure, null); });
                        return;
                    }
                    ...
                    webRequest.BeginGetResponse(getResponseResult =>
                    {
                        ...
                        {
                            Logger.AddEntry("ThreadCreatorService - Get failed @ GetResponse.");
                            dispatch.BeginInvoke(() => { result(ActionResult.Failure, null); });
                        }
                        else
                        {
                            Logger.AddEntry("ThreadReplyService - Get successful.");
                            AwfulThreadPage page = new AwfulThreadPreviewPage(html);
                            dispatch.BeginInvoke(() => { result(ActionResult.Success, page); });
                        }
        ...
        {
            Logger.AddEntry("ThreadCreatorService - GetRequest initiated.");
            ...
            try
            {
                ...
                postStream.Close();
                Logger.AddEntry("ThreadCreatorService - GetRequest successful.");
                return true;
            }
            catch (Exception ex)
            {
                Logger.AddEntry(string.Format("ThreadCreatorService - GetRequest failed: {0}", ex.Message));
                return false;
            }
        }
        ...
        {
            Logger.AddEntry("ThreadCreatorService - GetResponse initiated.");
            try
            {
                HttpWebRequest webRequest = result.AsyncState as HttpWebRequest;
                ...
                response.Close();
                Logger.AddEntry("ThreadCreatorService - GetResponse successful.");
                return true;
            }

            catch (WebException ex)
            {
                Logger.AddEntry(string.Format("ThreadCreatorService - GetResponse failed: {0}", ex.Message));
                return false;
            }
        }
