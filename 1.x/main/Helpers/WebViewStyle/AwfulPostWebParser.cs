...
namespace Awful.Helpers
{
    ...
    public class MainWebParser : AwfulPostWebParser
    {
        ...
        {
            body = ParseNode(node);
            body = ContentFilter.Censor(body);
        }
        ...
        public const string VIDEO_NODE_TAG = "iframe";
        ...
        private string HandleVideos(HtmlNode node)
        {
            if (node != null)
            {
                ...
                return string.Format("<a href=\"javascript:navigate('a{0}{1}')\">[click for video]</a>", Globals.Constants.DEMARC, src);
            }
            return node.OuterHtml;
        }
        ...
        {
            var embedNode = node.Descendants(VIDEO_NODE_TAG).FirstOrDefault();
            if (embedNode != null)
            {
    ...
    public class ImageWebParser : ShowHideWebParser
    {
        ...
        protected override string GetContent()
        {
            ...
            { 
                ...
                if (url.Contains("attachment.php"))
                {
                    url = String.Format("{0}/{1}", Globals.Constants.SA_BASE, url);
                }
            }
            catch (Exception) { }
            return url;
